...
class Venue( Base ):
    ...
    def __init__( self, foursq_id, name, verified, location_id ):
        self.foursq_id = foursq_id
        self.name = name
...
class Search( Base ):
    ...
    def __repr__( self ):
        return "<Search('%s','%.6f','%.6f')" % ( self.date, self.latitude. self.longitude )
...
class Lookup( Base ):
    ...
    id = Column( Integer, primary_key=True )
    venue_id = Column( Integer, ForeignKey('venues.id') )
    venue = relationship("Venue", backref=backref('lookups'), cascade="all, save-update")
    statistic_id = Column(Integer, ForeignKey('statistics.id'))
    statistic = relationship("Statistic", backref=backref('lookup'), uselist=False, cascade="all, save-update")
    expected_checkins = Column( Integer )
    actual_checkins = Column( Integer )
    date = Column( DateTime )

    def __init__( self, expected_checkins, actual_checkins, date ):
        self.expected_checkins = expected_checkins
        self.actual_checkins = actual_checkins
        self.date = date
